...
import numpy as np
import xarray as xr
from cinterop.cffi.marshal import TimeSeriesGeometryNative, as_bytes
...
def custom_wrap_cffi_native_handle(obj, type_id="", release_native=None):
    ...
    if __wrap_cffi_native_handle is None:
        raise RuntimeError(
            "The function creating custom wrappers around native objects is None: you must use set_wrap_cffi_native_handle to initialise it"
        )
...
def CreateEnsembleForecastTimeSeries_py(
    start: datetime, length: int, timeStepName: str
) -> "EnsembleForecastTimeSeries":
...
def GetDatasetEnsembleForecastTimeSeries_py(
    dataLibrary: "TimeSeriesLibrary", dataId: str
) -> "EnsembleForecastTimeSeries":
...
def SaveEnsembleForecastTimeSeriesToNetcdf_py(
    tsEnsTs: "EnsembleForecastTimeSeries", filename: str, overwrite: bool
) -> None:
...
def TimeSeriesFromTimeSeriesOfEnsembleOfTimeSeries_py(
    efts: "EnsembleForecastTimeSeries", indexInIssueTime: int, indexInForecastTime: int
) -> "TimeSeries":
    ...
    """
    efts_xptr = wrap_as_pointer_handle(efts)
    result = _TimeSeriesFromTimeSeriesOfEnsembleOfTimeSeries_native(
        efts_xptr.ptr, indexInIssueTime, indexInForecastTime
    )
    return custom_wrap_cffi_native_handle(result, "TIME_SERIES_PTR")
...
def SetItemEnsembleForecastTimeSeriesAsStructure_py(
    series: "EnsembleForecastTimeSeries", i: int, values: xr.DataArray
) -> None:
...
def _CreatePerfectForecastTimeSeries_native(observations, start, length, timeStepName, offsetForecasts, leadTime):
    result = uchronia_so.CreatePerfectForecastTimeSeries(
        observations, start, length, timeStepName, offsetForecasts, leadTime
    )
    return result


def CreatePerfectForecastTimeSeries_py(
    observations: "TimeSeries", start: datetime, length: int, timeStepName: str, offsetForecasts: int, leadTime: int
) -> "EnsembleForecastTimeSeries":
    ...
    start_datetime = marshal.datetime_to_dtts(start)
    timeStepName_c_charp = wrap_as_pointer_handle(as_bytes(timeStepName))
    result = _CreatePerfectForecastTimeSeries_native(
        observations_xptr.ptr, start_datetime.obj, length, timeStepName_c_charp.ptr, offsetForecasts, leadTime
    )
    # start_datetime - no cleanup needed?
    # no cleanup for const char*
    return custom_wrap_cffi_native_handle(result, "ENSEMBLE_FORECAST_TIME_SERIES_PTR")
...
def GetEnsembleForecastTimeSeriesGeometry_py(
    timeSeries: "EnsembleForecastTimeSeries", geom: TimeSeriesGeometryNative
) -> None:
...
def GetProviderTsGeometry_py(
    dataLibrary: "TimeSeriesProvider", variableIdentifier: str, geom: TimeSeriesGeometryNative
) -> None:
...
def GetProviderTimeSeriesValues_py(
    dataLibrary: "TimeSeriesProvider", variableIdentifier: str, values: np.ndarray, arrayLength: int
) -> None:
    ...
    variableIdentifier_c_charp = wrap_as_pointer_handle(as_bytes(variableIdentifier))
    values_numarray = marshal.as_c_double_array(values, shallow=True)
    _GetProviderTimeSeriesValues_native(
        dataLibrary_xptr.ptr, variableIdentifier_c_charp.ptr, values_numarray.ptr, arrayLength
    )
    # no cleanup for const char*
    # values_numarray - no cleanup needed?
