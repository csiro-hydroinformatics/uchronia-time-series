...
#     LoadEnsembleDataset_py(dataSetId, dataPath)
#   } else {
...
# #' 
# #' @export
# getDataSet(dataLibrary, dataId)
# {
#   result <- GetDatasetFromLibrary_Pkg_py(dataLibrary, dataId);
#   return(result)
# }
...
# #' @export
# getDatasetSingleTimeSeries(dataLibrary, dataId)
# {
#   result <- GetDatasetSingleTimeSeries_py(dataLibrary, dataId);
#   return(result)
# }
...
# #' @return an xts time series.
# #' @export
# getTimeSeriesFromProvider(provider, dataId) {
#   if(missing(dataId)) {
...
# #' @export
# getDataIdentifiers(provider) {
#   return(GetProviderTimeSeriesIdentifiers_py(provider))
# }
...
#       return(marshaledTimeSeriesToXts(TimeSeriesToTsInfo_Pkg_py(tsInfo)))
#     } else if(isEnsembleTimeSeries(tsInfo)) {
#       return(ensTsToXts(GetEnsembleTimeSeries_Pkg_py(tsInfo)))
#     } else {
#       stop(paste0('asXts: does not know how to convert to xts an object of external type "', tsInfo.type_id, '"'))
#     }
#   } else {
#     k <- class(tsInfo)
...
#   } else if(cinterop::isExternalObjRef(tsInfo)) {
#     if(isSingularTimeSeries(tsInfo)) {
#       return(tsIndex(GetTimeSeriesGeometry_Pkg_py(tsInfo)))
#     } else if(isEnsembleTimeSeries(tsInfo)) {
...
#       return(tsIndex(GetEnsembleForecastTimeSeriesGeometry_Pkg_py(tsInfo)))
#     } else {
