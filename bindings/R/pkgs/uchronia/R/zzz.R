...
.onLoad <- function(libname='~/R', pkgname='uchronia') {
  ...
  tryCatch(library.dynam('uchronia_r', pkgname, libname),  
    error = function(e) { 
      if(Sys.info()['sysname'] == 'Windows') {
        cat("Error loading shared library.\nYou may need to set up an environment variable LIBRARY_PATH.\nSee documentation at https://csiro-hydroinformatics.github.io/streamflow-forecasting-tools-onboard/blob/master/doc/install_windows.md#adding-environment-variable-library_path\n");
      } ; 
      stop(e) 
    } 
  )
